...
const VOUCHER_APPLIED_KEY = 'voucher-applied'; // Nowy klucz dla lokalnego stanu
...
export class CartService {
  ...
  constructor(private productService: ProductService) {
    ...
    this.voucherApplied = this.getVoucherAppliedFromStorage() || false; // Domyślnie zniżka nie jest jeszcze zastosowana
  ...
  sum = 0;
  voucherCode = '';
  voucherApplied = false; // Dodajemy nowe pole
  ...
  applyVoucher(voucherCode: string, sum: number): Observable<any> {
    ...
    if (voucherCode === 'vegan2024' && this.sum > 50) {
      ...
      this.saveVoucherAppliedToStorage(); // Zapisz stan zastosowania zniżki

      console.log('Voucher applied successfully. Discount applied: 10%');
      return of({ success: true, message: 'Voucher applied successfully.', discountedSum: this.sum });
    } else {
      console.log('Invalid voucher code or sum too low');
      return of({ success: false, message: 'Invalid voucher code or sum too low' });
    }
  }
  ...
  private getDiscountedSumFromStorage() {
    const discountedSum = localStorage.getItem(DISCOUNTED_SUM_KEY);
